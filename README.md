...
3. pin your current, working build once with `sudo ostree admin pin 0`
3. with that, you now need to run `rpm-ostree reset`
4. run a `reboot`
5. in fresh booted system, also pin your cleaned `rpm-ostree` setup as a base to revert to anytime: `sudo ostree admin pin 0` -- we do this to ensure no problems arise
