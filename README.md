...
6. run a podman build off your container file under sudo in your `Containerfile`'s directory:
`sudo podman build --pull=newer -t localhost/my-kinoite:latest .`
...
This way, to update your system you simply rebuild off your `Containerfile` and if you keep it with the same tag (acting as a release channel), you merely may run `sudo bootc update`. Ultimately, to ensure you don't lose your old custom container version, you should retag it by its image ID. For automating this, I built a script in my GitHub repo, here: 

In this setup, `rpm-ostree` is obsoleted for the most part except for seeing deployments. For example, to see deployments in `bootc` merely run `sudo bootc status` and to rollback to an older custom deployment it is as simple as `sudo bootc rollback`.
